# Write a program to play Mastermind with DQN
...
class Policy(nn.Module):
    ...
    def get_crosswise_embeddings(self, guess):
        style = torch.cat([guess[:3], guess[-1].unsqueeze(0)])
        style = self.style1(style)
        style = self.style2(style)

        subject = torch.cat([guess[3:6], guess[-1].unsqueeze(0)])
        subject = self.subject1(subject)
        subject = self.subject2(subject)

        setting = torch.cat([guess[6:9], guess[-1].unsqueeze(0)])
        setting = self.setting1(setting)
        setting = self.setting2(setting)

        return style, subject, setting

    def get_crosswise_embeddings2(self, guess):
        ...
        subject = self.subject1(subject)
        subject = self.subject2(subject).reshape(
            self.batch_size, self.n_guesses*16)

        setting = torch.cat((guess[:, :, 6:9], guess[:, :, -1:]), 2)
        setting = self.setting1(setting)
        setting = self.setting2(setting).reshape(
            self.batch_size, self.n_guesses*16)
    ...
    def forward(self, x):
        ...
        guess_embeddings = guess_embeddings.reshape(
            self.batch_size, 64*self.n_guesses)
        styles, subjects, settings = self.get_crosswise_embeddings2(x)
